package com.example.javaproject.haikang.sdk;   //修改包路径
...
public class ArtemisPostTest {
	...
	public static String callPostApiGetOrgList() throws Exception {
		...
		config.setHost("192.168.6.222"); // 平台nginx所在ip及https对应端口号
		config.setAppKey("29107259");  // 秘钥appkey
		config.setAppSecret("tJ6FMSrZo6avZR7iBhoh");// 秘钥appSecret
	...
	public static String callPostApiGetRegions() throws Exception {
		 ...
		 */
		ArtemisConfig config = new ArtemisConfig();
		config.setHost("192.168.6.222"); // 代理API网关nginx服务器ip端口
		config.setAppKey("29107259");  // 秘钥appkey
		config.setAppSecret("tJ6FMSrZo6avZR7iBhoh");// 秘钥appSecret
	 ...
	 * @throws Exception
	 */
	public static String getLiveViewUrl() throws Exception {
		...
		paramMap.put("cameraIndexCode", "2");
		paramMap.put("streamType", 0);
		paramMap.put("protocol", "websocket");
		paramMap.put("transmode", 1);
		paramMap.put("requestWebsocketProtocol", 0);
		String body = JSON.toJSON(paramMap).toString();
		Map<String, String> path = new HashMap<String, String>(2) {
			{
				put("https://", getCamsApi);
	...
	public static String getCameraInfo() throws Exception {
		/**
		 * https://ip:port/artemis/api/resource/v1/org/orgList
		 * 通过查阅AI Cloud开放平台文档或网关门户的文档可以看到获取组织列表的接口定义,该接口为POST请求的Rest接口, 入参为JSON字符串，接口协议为https。
		 * ArtemisHttpUtil工具类提供了doPostStringArtemis调用POST请求的方法，入参可传JSON字符串, 请阅读开发指南了解方法入参，没有的参数可传null
		...
		Map<String, Integer> paramMap = new HashMap<String, Integer>();// post请求Form表单参数
		paramMap.put("pageNo", 1);
...
//		System.out.println("------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		...
		String liveViewUrl = getLiveViewUrl();
